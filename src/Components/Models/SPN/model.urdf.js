export const exporter = () => {
  ...
  let PL = 10000; // Preset input parameters
  let new_height = 1000 * scaling_factor;

  let XY_scale = 1; // Mesh scaling parameters
  let Z_scale = 1;
  let faceplate_scale = 1;

  let default_height = 650 * scaling_factor; // Mesh placement parameters
  let base_height = 25 * scaling_factor;
  let middle_height = 513 * scaling_factor;
  let height_difference = 0;
  let top_placement = 0;
  let faceplate_placement = 0;

  let type = PL <= 1000 ? 0 : 1; // Set type based on payload input
  ...
  console.log("Faceplate", faceplate_placement);
  console.log("Base Height", base_height);
  console.log("Top Placement", top_placement);
  ...
  let createLinks = function () {
    ...
    return `
		...
		<link name="positioner_link">
			<visual>
				<geometry>
					<mesh filename="/SPN/meshes/Base_mesh.dae" scale="${XY_scale} ${XY_scale} 1.00000" />
				</geometry>
			</visual>
			<visual>
				<geometry>
					<mesh filename="/SPN/meshes/Middle_mesh.dae" scale="${XY_scale} ${XY_scale} ${Z_scale}" />
				</geometry>
				<origin xyz="0 0 ${base_height}" rpy="0 0 0" />
			</visual>
			<visual>
				<geometry>
					<mesh filename="/SPN/meshes/Top_mesh.dae" scale="${XY_scale} ${XY_scale} 1.00000" />
				</geometry>
		...
		<link name="faceplate_link">
			<visual>
				<geometry>
					<mesh filename="/SPN/meshes/Faceplate_mesh.dae" scale="${faceplate_scale} ${faceplate_scale} 1.00000" />
				</geometry>
			</visual>
		</link>
