...
const payload = 1500;
const height = 1250;
const fixture_distance = 1800;
const tailstock = 1;

export const exporter = () => {
  const body = ["Driver", "Idler"]; // Strings for link/joint functions
  const segment = ["Base", "Middle", "Top", "Faceplate"];
  const default_height = payload <= 7500 ? 1250 : 1500; // Set default rotational axis height based on user input
  const height_difference = height - default_height; // Determine change (+/-) in elevation to scale middle segments in z-axis
  ...
  const P2_dims = [
    447,
    fixture_distance + 368,
    368,
    520,
    height_difference + 250,
    480,
    height,
    520,
    height_difference + 410,
    320,
  ];
  const P3_dims = [
    556,
    fixture_distance + 548,
    548,
    650,
    height_difference + 250,
    600,
  ...
  let setPayloadVariables = function (input) {
    // This function can be used to output variables if they are dependant on the payload (add other set states to return_array)
    ...
    let output = type <= 3000 ? return_array[0] : return_array[1];
  ...
  var dim_array = [];
  switch (
    PL // New dimensional array used for origin/joint placement and scale factor calculations
  ) {
    case "P1":
      dim_array = P1_dims;
      break;
  ...
  let getScale = function (body) {
    // Function called by createLinks() to get appropriate scale factor for mesh files
  ...
  let createJoints = function () {
    // Function with for loop to create string of text required to create joints in urdf
    ...
    for (var i = 0; i < 8; i++) {
      // Each for loop creates one joint string

      if (i == 4) {
        // Reset q and n array counters based on i value
        q = 0;

        if (tailstock == 0) {
          // If tailstock is set to false then exit the for loop when the driver is complete
          break;
        }
      }
      if (i == 5) {
      ...
      origin =
        x_array[i] * scaling_factor + " 0 " + z_array[i] * scaling_factor;
  ...
  let createLinks = function () {
    // Function with for loop to create string of text required to create links in urdf
    ...
    for (var i = 0; i < 8; i++) {
      if (i == 4) {
        // Reset segment array counter and set body array counter to 1 (for idler)
        j = 0;
        k = 1;
      ...
      let mesh_name =
        "/Single/meshes/" + body[k] + "_" + segment[j] + "_" + PL + ".dae";

      let scale = 1;
      if (j == 1) {
        // Middle segment being placed > Call getScale() function
        scale = getScale(k);
      }
